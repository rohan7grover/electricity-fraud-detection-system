...
const GraphComponentWeekly = ({ consumer_number }) => {
  const userToken = localStorage.getItem('access');
  const [error, setError] = useState(null);

  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL}/get-consumption-history-weekly/${consumer_number}/`, {
      headers: {
  ...
  return (
    <div>
      {error ? (
        ...
        <canvas id="chartweekly" width="6" height="2"></canvas>
      )}
    </div>
  );
};

export default GraphComponentWeekly;
